...
public class ConsultasUpdatable {
    ...
    public void insertarEmpleadoViaResultSet() {
        ...
        try (Connection conn = ConexionBaseDatos.getConexion();
            ...
            // Establecer valores
            Empleado empleado = new Empleado("Nuevo","Empleado",35000.0,1);
            rs.updateString("nombre", empleado.getNombre() );
            rs.updateString("apellido", empleado.getApellido());
            rs.updateDouble("salario", empleado.getSalario());
            rs.updateInt("departamento_id", empleado.getDptmtId());
